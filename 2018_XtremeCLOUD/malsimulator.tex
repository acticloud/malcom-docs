...
%\end{verb}
%\caption{The top-20 {\sc mal} operators for TPCH}
...
i%(i.e. a list of {\sc mal} statements to be executed in that order), estimate 
...
%
%Orderby is translated into sort in {\sc mal}.
%
...
%Limit and sample operators(firstn, and sample? in {\sc mal}) are also trivial, we just output the minimum of the argumens size and the limit size.
%
...
%In {\sc malcolm} we use a kNN(k=5) on the joins we have in our dictionary, that operate on the same two columns if possible, and find the ones closest based on the argument sizes.
%...
